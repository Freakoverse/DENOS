/**
 * NIP-NMS — Phase 2: group creation, bootstrap & consent.
 *
 * Orchestration on top of the channel primitives in nms.ts:
 ...
 *   - Each member backs up its groups + H in a personal NIP-78 `nmsgc` event.
 *
 * Network publish/fetch is kept separate from the pure builders so the protocol logic
 * is testable without relays. See docs/NIP-NMS.md.
 */
...
export interface NmsGroup {
    groupNpub: string;
    groupPubkey: string; // hex
    hHex: string;        // group secret (local + nmsgc backup)
    initiator: string;   // hex pubkey
    members: NmsMember[];
    role: GroupRole;
    label?: string;
    createdAt: number;
    /** Verified cosigner xpubs collected from signed nms-xpub messages, keyed by member pubkey. */
    xpubs?: Record<string, CosignerXpub>;
    /** Encrypted group identity (name/picture/about) from the group kind:0. */
    profile?: GroupProfile;
}

export interface NmsInvite {
    groupNpub: string;
    groupPubkey: string; // hex
...
function npubToHex(npub: string): string {
    ...
    if (d.type !== 'npub') throw new Error('NMS: expected npub');
...
export function buildInviteWrap(invite: NmsInvite, initiatorSkHex: string, memberPubHex: string): Event {
    const rumor = {
        kind: 14,
...
/**
 * Parse + verify an incoming NIP-17 DM as an NMS invite. Returns null if it is not an
 ...
 */
export function parseInvite(wrap: Event, recipientSkHex: string): NmsInvite | null {
    try {
        const rumor = nip59.unwrapEvent(wrap, hexToBytes(recipientSkHex));
        const body = JSON.parse(rumor.content);
        if (body.type !== 'nms-invite' || typeof body.h !== 'string') return null;
...
/**
 * Scan recent NIP-17 gift wraps addressed to the user and return any that are NMS
...
export function fetchRecentInvites(
    ...
    return new Promise(resolve => {
        ...
        const subId = 'nmsinv_' + Math.random().toString(36).slice(2, 8);
        ...
        const finish = () => {
            ...
            const byGroup = new Map<string, NmsInvite>();
...
export async function createGroupAsInitiator(
    ...
    groupName?: string,
    relays: string[] = NMS_DEFAULT_RELAYS,
): Promise<{ group: NmsGroup; invitesSent: number }> {
    if (memberPubHexes.includes(initiatorPubHex)) {
        memberPubHexes = memberPubHexes.filter(p => p !== initiatorPubHex);
    }
    if (memberPubHexes.length === 0) throw new Error('NMS: select at least one other member');
    ...
    const nmsGroup: NmsGroup = {
        ...
        members: [
            ...
            ...memberPubHexes.map<NmsMember>(hex => ({ pubkey: hex, npub: hexToNpub(hex), status: 'invited' })),
        ],
        role: 'initiator',
        profile,
        createdAt,
    ...
    saveLocalGroup(initiatorPubHex, nmsGroup);
    ...
    if (profile) { try { await publishMembershipRecord(nmsGroup, relays); } catch { /* best-effort */ } }

    return { group: nmsGroup, invitesSent };
}
...
export async function acceptInvite(
    ...
    saveLocalGroup(selfPubHex, nmsGroup);
    const wrap = wrapGroupMessage({ type: 'nms-accept', content: {} }, selfSkHex, group);
...
export async function declineInvite(
    ...
    const group = deriveGroupKeypair(hexToBytes(invite.hHex));
    const wrap = wrapGroupMessage({ type: 'nms-decline', content: {} }, selfSkHex, group);
    await publishToRelays(wrap, relays);
}

/** Apply an inbound accept/decline channel message to a group's member roster (immutably). */
export function applyConsent(group: NmsGroup, msg: NmsUnwrapped): NmsGroup {
    if (msg.type !== 'nms-accept' && msg.type !== 'nms-decline') return group;
    const status: MemberStatus = msg.type === 'nms-accept' ? 'accepted' : 'declined';
    return {
        ...group,
...
export function isGroupReady(group: NmsGroup): boolean {
...
/** Send the active member's cosigner xpub to the channel as an inner-signed nms-xpub message. */
export async function sendCosignerXpub(
    ...
    const wrap = wrapGroupMessage(
        { type: 'nms-xpub', content: { xpub, fingerprint }, tags: [[TAG_XPUB, xpub, path]] },
        selfSkHex, gk,
    );
    return publishToRelays(wrap, relays);
}
...
/**
 * Parse a verified nms-xpub channel message into a cosigner key. The message's author was
 ...
 */
export function extractXpub(msg: NmsUnwrapped): CosignerXpub | null {
    if (msg.type !== 'nms-xpub') return null;
...
export function applyXpub(group: NmsGroup, x: CosignerXpub): NmsGroup {
...
export function allXpubsCollected(group: NmsGroup): boolean {
    const xpubs = group.xpubs ?? {};
    return group.members.every(m => !!xpubs[m.pubkey]);
}

/** Members still missing an xpub. */
export function missingXpubs(group: NmsGroup): NmsMember[] {
    const xpubs = group.xpubs ?? {};
...
export function cosignerList(group: NmsGroup): CosignerXpub[] {
...
async function publishPsbtMsg(
    group: NmsGroup, selfSkHex: string, uuid: string, approved: boolean,
    content: Record<string, unknown>, relays: string[],
): Promise<number> {
    const gk = deriveGroupKeypair(hexToBytes(group.hHex));
    const wrap = wrapGroupMessage(
        { type: 'nms-psbt', content, tags: [[TAG_PSBT, uuid], [TAG_APPROVED, approved ? 'yes' : 'no']] },
        selfSkHex, gk,
    );
    return publishToRelays(wrap, relays);
}
...
export function sendPsbtProposal(group: NmsGroup, selfSkHex: string, uuid: string, psbtBase64: string, summary: PsbtSummary, relays: string[] = NMS_DEFAULT_RELAYS) {
...
export function sendPsbtSignature(group: NmsGroup, selfSkHex: string, uuid: string, psbtBase64: string, relays: string[] = NMS_DEFAULT_RELAYS) {
    return publishPsbtMsg(group, selfSkHex, uuid, true, { psbt: psbtBase64 }, relays);
}
/** Decline a proposal. */
export function sendPsbtDecline(group: NmsGroup, selfSkHex: string, uuid: string, relays: string[] = NMS_DEFAULT_RELAYS) {
    return publishPsbtMsg(group, selfSkHex, uuid, false, {}, relays);
}
/** Announce that the finalized transaction was broadcast. */
export function sendPsbtBroadcast(group: NmsGroup, selfSkHex: string, uuid: string, txid: string, relays: string[] = NMS_DEFAULT_RELAYS) {
...
// verified per-member nms-xpub messages above, never from this blob.
export async function publishXpubsCache(group: NmsGroup, relays: string[] = NMS_DEFAULT_RELAYS): Promise<number> {
    const gk = deriveGroupKeypair(hexToBytes(group.hHex));
...
export function buildMembershipEvent(group: GroupKeypair, members: string[], initiator: string, profile: GroupProfile = {}, thresholdDefault = 2): Event {
    const nms = encryptToSelf(group.skHex, {
        name: profile.name ?? '',
    ...
    return finalizeEvent(
        { kind: KIND_GROUP_META, created_at: Math.floor(Date.now() / 1000), tags: [], content: JSON.stringify({ nms }) },
        group.sk,
    );
}

/** Publish the group kind:0 (membership + encrypted profile). Re-used for profile edits. */
export async function publishMembershipRecord(group: NmsGroup, relays: string[] = NMS_DEFAULT_RELAYS): Promise<number> {
    const gk = deriveGroupKeypair(hexToBytes(group.hHex));
...
export function fetchGroupProfile(group: NmsGroup, relays: string[] = NMS_DEFAULT_RELAYS): Promise<GroupProfile | null> {
    const gk = deriveGroupKeypair(hexToBytes(group.hHex));
    return new Promise(resolve => {
        const sockets: WebSocket[] = [];
        const subId = 'nmsp_' + Math.random().toString(36).slice(2, 8);
        ...
        const finish = () => {
            ...
            try {
                ...
                if (!parsed.nms) return resolve(null);
                const dec = decryptToSelf<GroupProfile>(gk.skHex, parsed.nms);
                resolve({ name: dec.name || undefined, picture: dec.picture || undefined, about: dec.about || undefined });
            } catch { resolve(null); }
        };
...
const localKey = (ownerHex: string) => `denos-nms-groups-${ownerHex}`;

export function getLocalGroups(ownerHex: string): NmsGroup[] {
    try {
        const raw = localStorage.getItem(localKey(ownerHex));
        return raw ? (JSON.parse(raw) as NmsGroup[]) : [];
    } catch {
        return [];
    }
}

export function saveLocalGroup(ownerHex: string, group: NmsGroup): void {
...
// ──────────────────────────────────────────────────────────────────────────
//  Personal recovery backup — NIP-78 `nmsgc`
// ──────────────────────────────────────────────────────────────────────────

interface NmsgcEntry {
    group_npub: string;
    h: string;
    role: GroupRole;
    label?: string;
...
export function serializeGroupIndex(groups: NmsGroup[]): NmsgcEntry[] {
    return groups.map(g => ({
...
/** Build the personal NIP-78 (`d=nmsgc`) backup event, encrypted to self. */
export function buildGroupIndexEvent(personalSkHex: string, groups: NmsGroup[]): Event {
...
// ──────────────────────────────────────────────────────────────────────────
//  Backup durability — cache signed nmsgc/msx events; rebroadcast if relays drop them.
...
function cacheBackupEvent(key: string, ev: Event): void {
    try { localStorage.setItem('denos-nms-backup-' + key, JSON.stringify(ev)); } catch { /* ignore */ }
}
function getBackupEvent(key: string): Event | null {
    try { const raw = localStorage.getItem('denos-nms-backup-' + key); return raw ? JSON.parse(raw) as Event : null; } catch { return null; }
}

/** Build, cache, and publish the personal nmsgc backup from the current local groups. */
export async function syncGroupIndex(personalSkHex: string, personalPubHex: string, groups: NmsGroup[], relays: string[] = NMS_DEFAULT_RELAYS): Promise<void> {
    const ev = buildGroupIndexEvent(personalSkHex, groups);
    cacheBackupEvent('nmsgc-' + personalPubHex, ev);
 ...
 */
export async function ensureBackupAlive(cacheKey: string, relays: string[] = NMS_DEFAULT_RELAYS): Promise<void> {
...
/** Rebroadcast all of this member's backups (nmsgc + each group's msx) if relays dropped them. */
export async function ensureBackupsAlive(personalPubHex: string, groupNpubs: string[], relays: string[] = NMS_DEFAULT_RELAYS): Promise<void> {
    await ensureBackupAlive('nmsgc-' + personalPubHex, relays);
    for (const npub of groupNpubs) await ensureBackupAlive('msx-' + npub, relays);
}

/** Fetch + decrypt the personal `nmsgc` backup into minimal group records (for recovery). */
export async function fetchGroupIndex(
    ...
    if (!ev) return [];
    const payload = decryptToSelf<{ groups: NmsgcEntry[] }>(personalSkHex, ev.content);
...
function fetchReplaceable(
    ...
    return new Promise(resolve => {
        const sockets: WebSocket[] = [];
        const subId = 'nmsf_' + Math.random().toString(36).slice(2, 8);
        let best: Event | null = null;
        let done = false;
        const finish = () => {
            if (done) return;
